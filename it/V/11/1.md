...
<ul>
	<li>
		<strong>avverso al rischio</strong> se preferisce ottenere con certezza il valore atteso di una lotteria, piuttosto che affrontare la lotteria stessa; cioè se $u(EP) > EU$. È facile vedere che ciò avviene se e solo se la funzione di utilità è <i>concava</i>, ovvero se $\pi < 1$.
	</li>
	<li>
	...
	<li>
		<strong>propenso al rischio</strong> se preferisce affrontare la lotteria, piuttosto che ricevere con certezza il suo valore atteso; cioè se $u(EP) < EU$. Questo avviene se e solo se la funzione è <i>convessa</i>, ovvero se $\pi > 1$.
	</li>
</ul>

Il grafico seguente illustra il concetto di utilità attesa e mostra come diverse forme della funzione di utilità (cioè diversi valori di $\pi$) corrispondano a diversi atteggiamenti nei confronti del rischio.
...
Per un individuo avverso al rischio, l'equivalente certo di una lotteria è sempre minore del payoff atteso della lotteria: $CE < EP$, e quindi $RP>0$. L’individuo è disposto a rinunciare a una parte del valore atteso pur di ottenere certezza. Inoltre, l'equivalente certo è tanto più piccolo (e quindi il premio al rischio tanto più grande) quanto più l'individuo è avverso al rischio. Infatti, per definizione di equivalente certo, abbiamo $CE=EU^{1/\pi}$, quindi $CE$ è tanto più piccolo (dunque $RP$ è tanto più grande) quanto più piccolo è il valore di $\pi$.
...
Concludiamo questa sezione con un'osservazione che ci tornerà utile più avanti, in particolare nel <a href="{{ site.baseurl }}/it/V/13">Capitolo 13</a>. Per un individuo con funzione di utilità concava (cioè avverso al rischio), vale una proprietà fondamentale: la presenza di variabilità nei payoff riduce l’utilità attesa. Questa proprietà può essere formulata in due modi equivalenti:
<ul>
	<li>
		A parità di payoff atteso, un individuo avverso al rischio preferisce lotterie meno variabili: tra due alternative con lo stesso payoff atteso, sceglierà quella con minore variabilità. 
	</li>
	<li>
		A parità di utilità attesa, se una lotteria è resa più variabile, allora l’individuo richiederà un valore atteso più elevato per accettarla. In altre parole, maggiore rischio deve essere compensato con maggiore payoff atteso.
	</li>
</ul>

Queste due formulazioni, che illustriamo nella figura seguente, esprimono lo stesso principio: la concavità della funzione di utilità implica che l’incertezza ha un costo. Questo principio è alla base di molte scelte economiche reali, come la domanda di assicurazione — di cui parleremo nella prossima sezione — o, come vedremo nel <a href="{{ site.baseurl }}/it/V/13">Capitolo 13</a>, il disegno ottimale di contratti di lavoro incentivanti.
